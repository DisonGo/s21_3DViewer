...
TagCounters CountTagsEdges(const string filePath) {
  FILE* obj_file = NULL;
  obj_file = fopen(filePath.c_str(), "r");
  if (!obj_file) throw std::invalid_argument("Can't open file");
  size_t linesz = 0;
...
OBJ ObjParser::ParseEdges(string filePath) {
  FILE* obj_file = NULL;
  obj_file = fopen(filePath.c_str(), "r");
  if (!obj_file) throw std::invalid_argument("Can't open file");
  OBJ obj;
  ...
  TagCounters tags = CountTagsEdges(filePath);
  ...
  obj.edges.insert(obj.edges.end(), faces, faces + tags.fC);
